...
# location <- c(site_data$Longitude[1], site_data$Latitude[1])
library(RgoogleMaps)
library(ggmap)
...
# site_data %>%
#   ggmap(myMAP) +
#   aes(x=site_data$Longitude, y=site_data$Latitude) +
#   geom_point()

ggmap(myMAP) + geom_point(aes(x=site_data$Longitude, y=site_data$Latitude), data = site_data, col = 'red')
# + geom_text_repel(aes(site_data$Longitude, site_data$Latitude, label=site_data$City))


# map("worldHires","USA", xlim=c(-141,-53), ylim=c(40,85), col="gray", fill=TRUE)
